...
class CommentsBloc extends Bloc<CommentsEvent, CommentsState> {
  ...
  CommentsBloc(this._postRepository) : super(const CommentsState.initial()) {
    on<CommentsEvent>((event, emit) async {
      await event.map(
        getComments: (e) async =>
            await _mapGetCommentsEventToState(e.postId, emit, reload: e.reload),
        selectCommentForReply: (e) async =>
            await _mapSelectCommentForReplyToState(e.comment, emit),
        getAComment: (e) async =>
  ...
  Future<void> _mapGetCommentsEventToState(
      String postId, Emitter<CommentsState> emit,
  ...
  Future<void> _mapGetACommentEventToState(
      String commentId, Emitter<CommentsState> emit) async {
  ...
  Future<void> _mapSaveACommentEventToState(
      SaveCommentPayload payload, Emitter<CommentsState> emit) async {
  ...
  Future<void> _mapCommentReactionEventToState(
      String commentId, String action, Emitter<CommentsState> emit) async {
  ...
  _mapSelectCommentForReplyToState(
      PostComment? comment, Emitter<CommentsState> emit) async {
